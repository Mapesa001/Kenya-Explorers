...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://kenya-explorers.onrender.com/api/users/login', {
        method: 'POST',
      ...
      <div className="relative z-10 w-full max-w-6xl bg-secondary/80 border-x-2 border-primary rounded-xl shadow-xl overflow-hidden grid md:grid-cols-2">
        ...
        <div className="p-8 sm:p-10" ref={formRef}>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full bg-primary hover:bg-secondary-900-brand hover:ring-2 hover:ring-primary text-white font-semibold py-3 rounded-md transition"
            >
              Log In
            </button>
